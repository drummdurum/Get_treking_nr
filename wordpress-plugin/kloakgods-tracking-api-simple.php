...
function kg_update_tracking(WP_REST_Request $request) {
    ...
    $tracking_url    = esc_url_raw($request->get_param('tracking_url'));
    ...
    // Nulstil check-tæller nu at tracking er fundet
    $order->delete_meta_data('_ao_tracking_check_count');
    $order->delete_meta_data('_ao_tracking_checked_at');

    $order->add_order_note(sprintf(
        'Tracking opdateret automatisk via Kloakgods bot. Carrier: %s | Nummer: %s',
        $carrier,
        $tracking_number
    ));
    $order->save();
